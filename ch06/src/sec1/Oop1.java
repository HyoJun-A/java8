...
public class Oop1 {
	...
	public static void main(String[] args) {
		...
		*/
		...
		//상속이 될 때 상속을 해주는 클래스를 슈퍼(super) 클래스라 하고, 상속을 받는 클래스는 서브(sub) 클래스라 한다.
		//상속 후에 메서드나 생성자 함수를 여러 가지 형태로 다르게 변경할 수 있난데 이를 메서드 오버라이딩과
		//하나의 클래스 내에서 메서드나 함수의 매개변수의 개수나 변수데이터타입에 따라 같은 이름으로 여러 개 지정이 가능한데 그것이 오버로딩
		//객체를 만들 때 여러 형태로 만들거나 선언할 수도 있고, 
		//메서드 오버라이딩이나 메서드 오버로딩과 같이 같은 이름의 여러 행태로 활용할 수 있음 -> 다형성
		//추상화 - 부모 클래스(인터페이스) 간소화된 꼭 필요한 요소만으로 구성하여 다른 서브 클래스들에서 공통으로 활용할 수 있도록 상세한 내용을 기술하지 않고 선언만 하는 것
